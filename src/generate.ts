...
export async function generateAnswer(prompt: PromptParts, opts: GenerateOptions = {}): Promise<GeneratedAnswer> {
  ...
  if (!apiKey) {
    ...
    const rawAnswer = `Leadership Insight: ${firstLines.slice(0, 300)} ... (stubbed; provide OPENAI_API_KEY for real generation) [#1]`;
  ...
  return { answer, citations, model };
    ...
    if (s.isNetworkError) {
      const firstLines = prompt.context.split(/\n/).slice(0, 3).join(' ');
      const rawAnswer = `Leadership Insight: ${firstLines.slice(0, 300)} ... (temporary offline; showing cached context) [#1]`;
      const citations = mergeCitationSources(extractContextCitations(prompt.context), extractAnswerInlineCitations(rawAnswer));
      const answer = rawAnswer.replace(/\[#\d+\]/g, '').replace(/\s{2,}/g, ' ').trim();
      return { answer, citations, model: 'stub-local' };
    }
...
export async function generateChatAnswer(prompt: PromptParts, history: ChatMessage[], opts: GenerateOptions = {}): Promise<GeneratedAnswer> {
  ...
  if (!apiKey) {
    ...
    const rawAnswer = `Coach: Let's explore that together. ${lastUser.slice(0, 160)} — Here are a few principles to consider based on the references above. [#1] (stubbed; set OPENAI_API_KEY for real generation)`;
    const citations = mergeCitationSources(extractContextCitations(prompt.context), extractAnswerInlineCitations(rawAnswer));
    const answer = rawAnswer.replace(/\[#\d+\]/g, '').replace(/\s{2,}/g, ' ').trim();
    return { answer, citations, model: 'stub-local' };
  }
